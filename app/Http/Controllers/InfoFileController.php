<?php
    ...
    class InfoFileController extends Controller
    {
        ...
        public function index()
        {
            ...
            $builder
                ->addTopButton(['name' => '添加软件信息', 'value' => 'create', 'url' => UrlFunc::jumpUrl('infofile/create'), 'type' => 'url']);
            ...
            $builder
                ...
                ->addTableColumn(['name' => '栏目', 'value' => 'classId', 'type' => 'state', 'extend' => [
                    'param' => $this->infoClassModel->getInfoListClass(4)
                ]])
            ...
            $builder->page($this->infoListModel->_list(4)->links());

            $builder->addListData($this->infoListModel->_list(4)->toArray()['data']);

            $builder
                ->addRightButton(['class' => 'btn-info', 'name' => '审核', 'value' => 'update', 'type' => FALSE, 'url' => UrlFunc::jumpUrl('infofile/check'), 'custom' => ['id'], 'way' => 'get'])
                ->addRightButton(['class' => 'btn-success', 'name' => '编辑', 'value' => 'edit', 'type' => FALSE, 'url' => UrlFunc::jumpUrl('infofile/edit'), 'custom' => ['id'], 'way' => 'get'])
                ->addRightButton(['class' => 'btn-primary', 'name' => '删除', 'value' => 'delete', 'type' => TRUE, 'url' => UrlFunc::jumpUrl('infofile/delete'), 'custom' => ['id'], 'way' => 'post']);
        ...
        public function create(Request $request)
        {
            if ($request->isMethod('post')) {
                ...
                $builder
                    ->addFormItem(['name' => 'redirect', 'type' => 'hidden', 'value' => UrlFunc::jumpUrl('infofile')])
                    ->addFormItem(['name' => 'classId', 'type' => 'select', 'title' => '栏目', 'parameter' =>
                        $this->infoClassModel->getInfoListClass(4), 'extra' => 'required'
                    ])
            ...
            if ($request->isMethod('post')) {
                ...
                $builder
                    ...
                    ->addFormItem(['name' => 'classId', 'type' => 'select', 'value' => $data['classId'], 'title' => '栏目', 'parameter' =>
                        $this->infoClassModel->getInfoListClass(4)
                    ])
